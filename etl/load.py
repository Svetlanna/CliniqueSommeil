...
def charger_raw_capteur(df_capteur, id_nuit):
    ...
    rows = [
        (
            ...
            row.get('spo2'),
            row.get('debit_nasal_pct'),
            row.get('effort_thoracique_pct'),
            row.get('position'),
...
def sauvegarder_resultats(indicateurs, id_nuit, df_capteur):
    ...
    plt.grid(True)
    plt.savefig(f"courbes/courbe_spo2_nuit_{id_nuit}.png")
    plt.close()
    ...
    plt.grid(True)
    plt.savefig(f"courbes/courbe_debit_nasal_nuit_{id_nuit}.png")
    plt.close()
    ...
    plt.grid(True)
    plt.savefig(f"courbes/ronflements{id_nuit}_vs_temps.png")
    plt.close()
